...
Style is **side-on**. Pixel craft: [`docs/PIXEL_CRAFT.md`](docs/PIXEL_CRAFT.md). Side-on refs: [`docs/refs/sideon.md`](docs/refs/sideon.md). Default palette is EDG32. Before drawing: canvas size + palette (bundle / `.gpl` / hex / palette from a `.aseprite`). Paint with 1px lines, clusters, sel-out; do not use `U()`/`S()` or `outline_from_volume` as the finished outline. No copyrighted game assets in `work/`.
